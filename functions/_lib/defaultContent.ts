// 편집 가능한 콘텐츠 기본값 — KV가 비어있을 때 사용 (API/관리자 공통 시드)
...
export const defaultContent: SiteContent = {
  ...
  faqs: [
    ...
    { id: 'f2', q: '가맹 계약부터 매장 오픈까지 얼마나 걸리나요?', a: '상담일로부터 매장 오픈까지 약 45일이 소요됩니다. 상담·계약(정보공개서 제공 14일 후 계약) → 점포 실측·설계 → 인테리어 공사(약 20일) → 개점 교육(약 10일) → 오픈 순으로 진행됩니다.' },
...
export async function loadContent(kv: KVNamespace): Promise<SiteContent> {
  const saved = await kv.get(KEY, 'json')
  if (!saved) return defaultContent
  // 누락 필드는 기본값으로 보강
  const s = saved as Partial<SiteContent>
  return {
    menus: s.menus ?? defaultContent.menus,
    stores: s.stores ?? defaultContent.stores,
    instagram: s.instagram ?? defaultContent.instagram,
    faqs: s.faqs ?? defaultContent.faqs,
  }
}

export async function saveContent(kv: KVNamespace, content: SiteContent): Promise<void> {
